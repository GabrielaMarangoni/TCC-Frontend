...
import { HStack, Stack, Tab, Tabs, TabList, TabPanels, TabPanel } from "@chakra-ui/react";
    ...
    <div>
      ...
      <div className="w-4/5 bg-white ml-80 px-32 pt-24">
        <HStack justifyContent="space-between">
          <h1 className={textTitle}>Observações</h1>
      ...
      <div className={divGeneral}>
        <div className="pb-20">
          <Stack spacing={5}>
            <Tabs size="lg" onChange={(index) => setTabSelected(index)}>
              <TabList>
                <Tab value="1" className="w-fit">Minhas observações</Tab>
                <Tab value="2" className="w-fit">Todas as observações</Tab>
                {tabSelected === 0 && (
                  <div className="flex ml-auto items-center justify-end">
                    <h1> Total: {myObservations.filter((el: any) => text.length ? el.OBS.toLowerCase().indexOf(text.toLowerCase()) !== -1 : el).length}</h1>
                  </div>
                )}
                {tabSelected === 1 && (
                  <div className="flex ml-auto items-center justify-end">
                    <h1>Total: {data.filter((el: any) => {
                    ...
                    }).filter((el: any) => text.length ? el.OBS.toLowerCase().indexOf(text.toLowerCase()) !== -1 : el).length}</h1>
                  </div>
                )}
              </TabList>
             ...
             { <TabPanels>
                <TabPanel>
                  <Stack spacing={5} className="overflow-y-auto">
                    {myObservations.filter((el: any) => text.length ? el.OBS.toLowerCase().indexOf(text.toLowerCase()) !== -1 : el).map((e: any, index: number) => {
                      return (
                        <div
                          key={index}
                          className={cn("border-2 border-button p-5 rounded-xl", e.ID_PROF === localStorage.getItem("professor_id") ? "" : "")}
                        > 
                          <HStack justifyContent="space-between">
                            <div
                            ...
                            <div className="flex pb-3 space-x-10">
                              {e.GRUPOS.length > 0 && e.GRUPOS.filter((el: any) => {
                                const grupo = el.PROFESSOR[0].ID === localStorage.getItem("professor_id")
                                if (grupo) {
                                  return grupo
                                }
                              }).map((grupo: any) => (
                              ...
                              <DeleteButton
                                onClick={() => handleDeleteObservation(e.ID_OBS)}
                              />
                            </div>
                          </HStack>
                          <div className={cn("bg-slate-100 p-4 rounded-lg ", e.ID_PROF === localStorage.getItem("professor_id") ? "" : "")}>
                      ...
                      {myObservations.filter((el: any) => text.length ? el.OBS.toLowerCase().indexOf(text.toLowerCase()) !== -1 : el).length === 0 && (
                        <h1 className="col-span-3 text-gray-500 text-center font-semibold text-lg">
                ...
                <TabPanel>
                  <Stack spacing={5} className="overflow-y-auto">
                    ...
                    {data.filter((el: any) => {
                      ...
                      <div
                        key={index}
                        className={cn("border-2 border-button p-5 rounded-xl", e.ID_PROF === localStorage.getItem("professor_id") ? "bg-yellow-100" : "")}
                      >
                          <HStack justifyContent="space-between">
                            <div
