...
const TableBase: React.FC<TabelBaseProps> = ({
  ...
  const getAlign = (col: TColumn) => {
    ...
    if (align === TColumnAlign.right || col.align === TColumnAlign.right) {
      return "text-right";
    }
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left">
                {columns.map((column) => (
                  <th
                    className={cn(column.align === TColumnAlign.right ? "text-right" : "text-left", "py-2 px-5 top-0 border-b border-gray-200 bg-buttonHover")}
                    key={column.header}
                  >
                    <div>
                      <span>
