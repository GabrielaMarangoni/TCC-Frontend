...
const TableButton: React.FC<TableButtonProps> = ({
  ...
  function getAluno(row: any){
    router.push(`/geral/alunos/historico?RA=` + row.RA)
  }
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left">
                {columns.map((column) => (
                  <th
                    className="py-2 px-5 top-0 border-b border-gray-200 bg-buttonHover"
                    key={column.header}
                  >
                    <div>
                      <span>
            ...
            <tbody className="divide-y divide-gray-200">
              ...
              {data.map((row) => {
                return (
                  <tr key={row[uniqueKey]} className="cursor-pointer hover:bg-button"
                  onClick={() => getAluno(row)}>
                    {columns.map((column) => {
