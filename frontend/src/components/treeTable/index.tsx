...
import { TColumn } from "../Table/TColumn";
...
const TreeNode: React.FC<TreeNodeProps> = ({
  ...
  const handleCheckboxChange = () => {
    ...
    setSelecteds((prevSelecteds: any[]) => {
      if (!isChecked) {
        const descendants = collectDescendants(data);
        return [...prevSelecteds, data, ...descendants];
      } else {
        const descendants = collectDescendants(data);
        return prevSelecteds.filter(
      ...
      <tr>
        ...
        >
          <td className=" py-2 align-middle text-left flex">
            ...
            />
            <button
              className="flex align-center justify-center items-center mr-10 text-lg"
          ...
          {isParent && (
            <td>
              <button
                ...
                className="flex  px-2 hover:text-button "
              >
                <svg
        ...
        {isParent && <hr className="border border-textBlack13 my-1" />}
      </tr>
      {data?.children &&
...
const TreeTable: React.FC<TreeTableProps> = ({ datat, columns, uniqueKey }) => {
  ...
  return (
    <div className="w-full mx-auto  border border-black h-4/5 rounded-lg">
      <div className="w-full overflow-hidden rounded-lg">
        <div className="w-full overflow-x-auto">
          <table className="w-full">
            <tbody className="">
              <tr>
                <td className="flex pt-3 p-2">
                  <input
                    type="checkbox"
                    checked={allSelected}
                    onChange={() => {
                  ...
                  <td>
                    {showButton && ( // Renderiza o botão apenas quando showButton for true
